...
@section('content')
    ...
    <div class="card text-center">
        <div class="card-header">
            {{ __('strings.series_card_header') }}
        </div>
        <div class="card-body">
            <p class="card-text">{{ __('strings.series_card_text') }}</p>
            <a href="{{ route('series.index') }}" class="btn btn-primary">{{ __('strings.list_title.series') }}</a>
        </div>
    </div>
@endsection
